...
// ─── About Easy Bridge dialog ─────────────────────────────────────────────────
class BridgeAboutDialog final : public juce::DocumentWindow
{
public:
    static void show (juce::Component* relativeTo = nullptr,
                      ...
                      juce::Font fontLight = {})
    {
        new BridgeAboutDialog (relativeTo, fontBold, fontLight);
    }

    void closeButtonPressed() override
    {
        juce::MessageManager::callAsync (
            [safe = juce::Component::SafePointer<BridgeAboutDialog> (this)]
            {
...
private:
    BridgeAboutDialog (juce::Component* relativeTo, juce::Font fontBold, juce::Font fontLight)
        : juce::DocumentWindow ("About Easy Bridge",
    ...
    {
        BridgeAboutDialog& owner_;
        juce::Font   fontBold_;
        juce::Font   fontLight_;
        juce::TextButton btnClose_  { "Close" };
        juce::TextButton btnGithub_ { "GitHub" };

        Content (BridgeAboutDialog& o, juce::Font fontBold, juce::Font fontLight)
            : owner_ (o),
              ...
              fontLight_ (fontLight.getHeight() > 0.0f ? fontLight : juce::Font (juce::FontOptions (30.0f)))
        {
            ...
            btnGithub_.onClick = []
            {
                juce::URL ("https://github.com/LD99VT/EASY-BRIDGE").launchInDefaultBrowser();
            };
        ...
        void paint (juce::Graphics& g) override
        {
            ...
            const juce::String bridgeStr = "BRIDGE";
            const float easyW   = fBold.getStringWidthFloat (easyStr);
            const float bridgeW = fLight.getStringWidthFloat (bridgeStr);
            const float totalW  = easyW + bridgeW;
            ...
            g.drawText (bridgeStr, (int) (startX + easyW), titleY, (int) bridgeW + 2, (int) titleH, juce::Justification::centredLeft);

            // ── Version ───────────────────────────────────────────────────────
            g.setFont (juce::FontOptions (13.0f));
            g.setColour (juce::Colour::fromRGB (0x8a, 0x8a, 0x8a));
            g.drawText ("v" + juce::String (version::kAppVersion),
                        0, 64, w, 18, juce::Justification::centred);

            // ── Description ───────────────────────────────────────────────────
            //
            // Easy Bridge converts your selected timecode source (LTC, MTC,
            // ArtNet, OSC) to any combination of outputs: LTC, MTC, ArtNet.
            // The application is written in C++ using the JUCE framework and
            // the Steinberg ASIO SDK for ultra-low-latency audio I/O.
            //
            g.setFont (juce::FontOptions (13.0f));
            g.setColour (juce::Colour::fromRGB (0xc0, 0xc0, 0xc0));
            const juce::String line1 =
                "Converts your selected timecode source to any other format.";
            const juce::String line2 =
                u8"In: LTC \u2022 MTC \u2022 ArtNet \u2022 OSC  \u2192  Out: LTC \u2022 MTC \u2022 ArtNet";
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BridgeAboutDialog)
};
